...
mypath3 = '[path to HRRR data]'
mypaths = [mypath1, mypath2, mypath3]
...
#    date_first = filename_first.split('/')[-1].split('.')[1][3:10+1]
    if filename_first[0:3+1] == 'mrms':
        ...
        date_first = '[depends on path/filename]'
...
#    date_last = filename_last.split('/')[-1].split('.')[1][3:10+1]
    if filename_first[0:3+1] == 'mrms':
        ...
        date_last = '[depends on path/filename]'
...
date_first = min(date_firsts)
date_first_str = date_firsts_strs[np.where(date_firsts == date_first)]
date_last = min(date_lasts)
date_last_str = date_lasts_strs[np.where(date_lasts == date_last)]
...
#        date_myvar = date_myvar[0:3+1] + date_myvar[4:5+1] + date_myvar[6:7+1] + '-' + date_myvar[9:10+1]
            ...
            date_myvar = '[depends on path/filename]'
...
#            print("The time spent iterating through the grid points is :", timeit.default_timer() - starttime)
            else:  # if the myvar is HRRR or RRFS
                testvarMap1 = testvarread['depends on HRRR variable name'][np.arange(maptedge, mapbedge, 10), np.arange(mapledge, mapredge, 10)]
                testvarMap2 = testvarread['depends on HRRR variable name'][np.arange(maptedge, mapbedge, 10), np.arange(mapledge, mapredge, 10)]
                vectorOfMap1 = testvarMap1.stack(point=["latitude", "longitude"])  # variable names depend on HRRR
                vectorOfMap2 = testvarMap2.stack(point=["latitude", "longitude"])  # variable names depend on HRRR
                dataArray.append(dataArrays[0][dateCount] - vectorOfMap)  # 01h MRMS QPE - 01h model QPF
                dataArray2.append(dataArrays[1][dateCount] - vectorOfMap)  # 24h MRMS QPE - 24h model QPF
                indices_ari.append(dateCount)  # while the model data is not being filtered by ARI, the indices are still needed to ensure data is available for all variables
                datetimes_ari.append(datetimes_full[dateCount])
                count_ari = count_ari + 1
    ...
    elif myfiles[myvar][0][0:3+1] != 'mrms':
        dataArrays.append(np.array(dataArray2)  # if using model data, add the second QPF variable
...
with open('SOM_' + SOMsize_str + '_multivar_' + myregion + '_precip_thresh_count_ari' + ari + '_obsvsmodel.pkl', 'wb') as f:  # Python 2: open(..., 'w')
