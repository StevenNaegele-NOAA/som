...
from sklearn_som.som_smn import SOM
...
SOMsize_str = '3x2'#'5x5'#'3x2'#'4x3'#'5x5'
myregion = 'SE'
ari = '1yr'
...
with open('/scratch1/BMC/wrfruc/naegele/SOM_3x2_multivar_' + myregion + '_precip_thresh_count_ari' + ari + '_2021.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
...
def mysom2_multivar_thresh_newdomains_sklearn_noARI_test(myfiles, date_first, date_last, varnames, SOMsize, mapedges, myregion, d):
...
#        shared_indices = [datetimes_common.index(x) for x in set(shared_datetimes)]  # get the indices (for the current variable's dataArray) of the datetimes shared among all variables  #ValueError: '20191003-18' is not in list
        shared_indices = [datetimes_common.index(x) for x in set(datetimes_common)]  # test "turning off" ARI filtering by using all available data
        dataArray = dataArray[shared_indices]  # IndexError: index 32772 is out of bounds for axis 0 with size 9934  --> 32772 is first of shared_indices
...
#    varSOM = SOM(SOMsize[0], SOMsize[1], dim=len(varnames)*np.prod([math.ceil((mapredge-mapledge)/10), math.ceil((maptedge-mapbedge)/10)]), sigma=1.75, lr=0.875, max_iter=500)
    varSOM = SOM(SOMsize[0], SOMsize[1], dim=len(varnames)*np.prod([math.ceil((mapredge-mapledge)/10), math.ceil((maptedge-mapbedge)/10)]), sigma=1.75, lr=0.975, max_iter=500)
#    varSOM = SOM(SOMsize[0], SOMsize[1], dim=len(varnames)*np.prod([math.ceil((mapredge-mapledge)/10), math.ceil((maptedge-mapbedge)/10)]), sigma=0.75, lr=0.125, max_iter=500)
...
#    varSOM.fit(xr.DataArray(dataArray_train))
    weights = varSOM.fit(xr.DataArray(dataArray_train))
...
#    print(predictions)
    ...
    print('sys.getsizeof(dataArrays_orig) = ' + str(sys.getsizeof(dataArrays_orig)))
    ...
    print('sys.getsizeof(dataArrays_orig[0]) = ' + str(sys.getsizeof(dataArrays_orig[0])))
    print('sys.getsizeof(dataArray_train[0]) = ' + str(sys.getsizeof(dataArray_train[0])))
    print('sys.getsizeof(dataArrays_orig[0][0]) = ' + str(sys.getsizeof(dataArrays_orig[0][0])))
...
#    print('len(varSOM) = ' + str(len(varSOM)))
    ...
    print('len(dataArrays_orig) = ' + str(len(dataArrays_orig)))
...
#    print('len(predictions[0]) = ' + str(len(predictions[0])))
    print('len(dataArrays_orig[0]) = ' + str(len(dataArrays_orig[0])))
...
#    print('type(varSOM) = ' + str(type(varSOM)))
    ...
    print('type(dataArrays_orig) = ' + str(type(dataArrays_orig)))
...
#    print('type(predictions[0]) = ' + str(type(predictions[0])))
    print('type(dataArrays_orig[0]) = ' + str(type(dataArrays_orig[0])))
    print('type(dataArray_train[0]) = ' + str(type(dataArray_train[0])))

    print('len(dataArrays_orig[0][0]) = ' + str(len(dataArrays_orig[0][0])))
    print('type(dataArrays_orig[0][0]) = ' + str(type(dataArrays_orig[0][0])))
...
#    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_sklearn_2020_2023.pkl', 'wb') as f:  # Python 2: open(..., 'w')
    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_sklearn_lr975_6hr_test.pkl', 'wb') as f:  # Python 2: open(..., 'w')
...
#        pickle.dump([varSOM, predictions, dataArrays_orig, dataArray_train, mapedges, myfiles, varnames, SOMsize, mapedges, myregion], f)
        pickle.dump([varSOM, weights, predictions, dataArrays_orig, dataArray_train, mapedges, myfiles, varnames, SOMsize, mapedges, myregion], f)
        # To open the file later
        # with open('objs.pkl', 'rb') as f:  # Python 2: open(..., 'r') Python 2: open(..., 'rb')
        #     obj0, obj1, obj2, ... = pickle.load(f)
...
#    return varSOM, predictions, dataArrays, map_lats, map_lons, dataArray_train
    return varSOM, weights, predictions, dataArrays, map_lats, map_lons, dataArray_train
