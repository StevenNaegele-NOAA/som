...
myregion_plot = 'SE'#str(sys.argv[1])
...
#precip_duration = precip_duration1
#varname_pickle = 'radaronly_QPE_01H_24H'  # variable name for the pickle file where output from this script will be saved
varname_pickle1 = 'radaronly_QPE_' + precip_duration1  # variable name for the pickle file where output from this script will be saved
varname_pickle2 = 'radaronly_QPE_' + precip_duration2  # variable name for the pickle file where output from this script will be saved
varname_title = 'QPE ' + precip_duration1 + ' & ' + precip_duration2
output_interval1 = 6
output_interval2 = 24
#output_interval = output_interval1
...
#    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion_plot + '_vars_batch_newdomains_sklearn.pkl', 'rb') as f:  # Python 2: open(..., 'r')
    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_' + varname_pickle1 + '_' + myregion_plot + '_vars_batch_newdomains_sklearn_' + str(output_interval1) + 'hr_test.pkl', 'rb') as f:  # Python 2: open(..., 'r')
...
#        varSOM, predictions, dataArrays_orig, dataArray_train, mapedges, myfiles, varnames, SOMsize, mapedges, myregion = pickle.load(f)
        varSOM1, predictions1, dataArray_orig1, dataArray_train1, mapedges, myfiles1, varname1, SOMsize, mapedges, myregion = pickle.load(f)

    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration2 + '_' + varname_pickle2 + '_' + myregion_plot + '_vars_batch_newdomains_sklearn_' + str(output_interval2) + 'hr_test.pkl', 'rb') as f:  # Python 2: open(..., 'r')
        varSOM2, predictions2, dataArray_orig2, dataArray_train2, mapedges, myfiles2, varname2, SOMsize, mapedges, myregion = pickle.load(f)

#   sample filename: /scratch2/BMC/wrfruc/naegele/SOM_3x2_thresh_ari1yr1hr_radaronly_QPE_1hr_SE_vars_batch_newdomains_sklearn_6hr_test.pkl
#    with open('/scratch1/BMC/wrfruc/naegele/SOM_3x2_multivar_' + myregion + '_precip_thresh_count_ari' + ari + '_6hr.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
    with open('/scratch1/BMC/wrfruc/naegele/SOM_3x2_' + precip_duration1 + 'QPE_' + myregion_plot + '_precip_thresh_count_ari' + ari + '_' + str(output_interval1) + 'hr.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
#        varnames, myfiles, myregion, indices_ari_allvars, datetimes_ari_allvars, count_ari_allvars = pickle.load(f)
        varname1, myfiles1, myregion, indices_ari1, datetimes_ari1, count_ari1, precip_duration1 = pickle.load(f)
#    print('datetimes_ari_allvars[0][0] = ' + str(datetimes_ari_allvars[0][0]))  # 20150220-18

    with open('/scratch1/BMC/wrfruc/naegele/SOM_3x2_' + precip_duration2 + 'QPE_' + myregion_plot + '_precip_thresh_count_ari' + ari + '_' + str(output_interval2) + 'hr.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
        varname2, myfiles2, myregion, indices_ari2, datetimes_ari2, count_ari2, precip_duration2 = pickle.load(f)
#    print('datetimes_ari_allvars[0][0] = ' + str(datetimes_ari_allvars[0][0]))  # 20150220-18

    print('indices_ari1 = ')
    print(indices_ari1)
    print('len(indices_ari1) = ' + str(len(indices_ari1)))

    print('datetimes_ari2 = ')
    print(datetimes_ari2)
    print('len(datetimes_ari1) = ' + str(len(datetimes_ari1)))
#    print('len(count_ari1) = ' + str(len(count_ari1)))  # int(???)

    print('len(indices_ari2) = ' + str(len(indices_ari2)))
    print('len(datetimes_ari2) = ' + str(len(datetimes_ari2)))
#    print('len(count_ari2) = ' + str(len(count_ari2)))  # int(???)

    datetimes_ari1_myd = [x[0:7+1] for x in datetimes_ari1]

    datetimes_from2_in1 = []
    for datetime_ari2 in datetimes_ari2:
        if datetime_ari2 in datetimes_ari1:
            datetimes_from2_in1.append(datetime_ari2)
    indices_ari1_in2_mask = np.in1d(datetimes_ari1, datetimes_from2_in1)
    indices_ari1_in2 = np.array(indices_ari1)[indices_ari1_in2_mask]
...
###
    sample_filename = '/scratch1/BMC/wrfruc/ejames/climo/mrms/202212/mrms_radaronly_2022123118.nc'
    filename_first = myfiles1[0][0]#[varcount][0]
...
#        date_first = filename_first.split('/')[-1].split('.')[1][3:10+1]
    ...
    date_first = date_first[0:7+1] + '-' + '12'#date_first[8:9+1]
    print('date_first = ' + str(date_first))
    filename_last = myfiles1[0][-1]#[varcount][-1]
...
#        date_last = filename_last.split('/')[-1].split('.')[1][3:10+1]
    ...
    date_last = date_last[0:7+1] + '-' + '12'#date_last[8:9+1]
...
#    datetimes_full = pandas.date_range(date_first,date_last,freq='24h')
#    datetimes_full = pandas.date_range(date_first,date_last,freq=str(output_interval)+'h')
...
#    shared_datetimes = set(datetimes_ari_allvars[0])  # start the list of datetimes where the threshold is met for at least one variable, with the datetimes for the first variable (converted to a set)
    shared_datetimes = set(datetimes_ari1)
...
#        datetimes_in_2nd_set_but_not_1st = set(datetimes_ari_allvars[myvar]) - shared_datetimes  # subtract the set of datetimes for myvar from the set of datetimes for the variables represented by shared_datetimes
        datetimes_in_2nd_set_but_not_1st = set(datetimes_ari2) - shared_datetimes  # subtract the set of datetimes for myvar from the set of datetimes for the variables represented by shared_datetimes
...
#            dataArray_node = dataArray_orig[winning_node_indices]
...
#    plt.savefig('SOM_' + varname + '_' + varname + 'train_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion_plot + '_test2_fixedtest2_clean_sklearn_6hr.png')



    print('indices_ari1_in2 = ' )
    print(indices_ari1_in2)  # indices_ari1_in2 = [    7    11    15 ... 11475 11479 11483]
    print('len(indices_ari1_in2) = ' + str(len(indices_ari1_in2)))  # 2364
    print('len(predictions1) = ' + str(len(predictions1)))  # 11137

    fig = plt.figure(figsize=(50, 10))
    counts_som1_som2 = np.zeros((np.prod(SOMsize), np.prod(SOMsize)))
    som2_index = -1
    for myindex in indices_ari1_in2:
        som2_index = som2_index + 1
        counts_som1_som2[predictions1[myindex], predictions2[som2_index]] = counts_som1_som2[predictions1[myindex], predictions2[som2_index]] + 1
    title_fontsize = 28
    axis_fontsize = 18
    annot_fontsize = 14
    np.set_printoptions(threshold=sys.maxsize)
    print('counts_som1_som2 = ' + str(counts_som1_som2))
#        ax = sns.heatmap(monthly_sums, annot=True, fmt='g', xticklabels=year_month_strs_allvars[varcount], yticklabels=np.arange(1, np.prod(SOMsize)+1), annot_kws={'size': annot_fontsize}, cbar_kws={'pad': 0.01})
    ax = sns.heatmap(counts_som1_som2, annot=True, fmt='g', xticklabels=np.arange(1, np.prod(SOMsize)+1), yticklabels=np.arange(1, np.prod(SOMsize)+1), annot_kws={'size': annot_fontsize}, cbar_kws={'pad': 0.01})
        #ax.set_xlabel('month', fontsize=axis_fontsize)
    ax.set_xlabel(varname_pickle1 + ' SOM node', fontsize=axis_fontsize)
    ax.set_ylabel(varname_pickle2 + ' SOM node', fontsize=axis_fontsize)
...
#        plt.title('Maps per SOM node per month -- ' + varname_title + ' (lr=0.25)', fontsize=title_fontsize)
    plt.title('Maps in common between SOMs -- ' + varname_title, fontsize=title_fontsize)
...
#        plt.savefig('SOM_heatmap_' + myvarname + '_01H24Htrain_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion + '_test2_fixedtest2_clean_sklearn_6hr.png', bbox_inches='tight', pad_inches=0.01)
    plt.savefig('SOM_heatmap_' + precip_duration1 + 'QPE' + str(output_interval1) + 'out_vs_' + precip_duration2 + 'QPE' + str(output_interval2) + 'out_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion_plot + '_test2_fixedtest2_clean_sklearn_test.png', bbox_inches='tight', pad_inches=0.01)


### for date2 in dates2:
###     dates1_for_date2 = np.array(dates1)[np.where(np.array(dates1_myd) == date2[0:7+1])[0]]

    datetimes_ari1_in_ari2period_all = []
    datetimes_ari1_in_ari2period_all_counts = []
    for datetime_ari2 in datetimes_ari2:
        datetimes_ari1_in_ari2period = np.array(datetimes_ari1)[np.where(np.array(datetimes_ari1_myd) == datetime_ari2[0:7+1])[0]]
        datetimes_ari1_in_ari2period_all.append(datetimes_ari1_in_ari2period)
        datetimes_ari1_in_ari2period_count = len(datetimes_ari1_in_ari2period)
        datetimes_ari1_in_ari2period_all_counts.append(len(datetimes_ari1_in_ari2period))
...
#    for myfile in datetimes_ari:
#    for myfile in shared_datetimes:
    for myfile in datetimes_ari2:  # needed because I want to determine the number of datetimes in variable 1 that occur in the same period represented by variable 2's datetime
#            myyear = myfile.split('/')[-1].split('.')[1][3:6+1]
#            myyear = myfile.split('/')[-1].split('_')[-1][0:3+1]
        myyear = myfile[0:3+1]
#            mymonthname = d[myfile.split('/')[-1].split('.')[1][7:8+1]]
...
#            elif mymonthname == 'Dec' and int(myyear) == years_doubles_allvars[varcount][-1]:
    ...
    print('seasons_all = ' + str(seasons_all))
    node_count = -1
...
#                winning_node_indices = winning_nodes[jSOM, iSOM]
            ...
            node_count = node_count + 1
            print('len(predictions1) = ' + str(len(predictions1)))
            winning_node_indices1 = np.where(predictions1 == node_count)  # same as above, but for sklearn-som
            print('len(winning_node_indices1) [again] = ' + str(len(winning_node_indices1)))
            print('np.array(winning_node_indices1)[0] [again] = ')# + str(winning_node_indices))
            print(np.array(winning_node_indices1)[0])
#                winning_node_indices = np.nonzero(np.in1d(winning_node_indices,datetime_6hr_indices_allvars[varcount]))[0]  # now only use indices for times that are in 6-hour intervals
#                winning_node_indices = np.array(winning_node_indices)[0][np.in1d(winning_node_indices,datetime_6hr_indices_allvars[varcount])]  # now only use indices for times that are in 6-hour intervals
#                datetime_6hr_indices_mask = np.in1d(winning_node_indices,datetime_6hr_indices_allvars[varcount])  # now only use indices for times that are in 6-hour intervals
#                winning_node_indices = winning_node_indices[datetime_6hr_indices_mask]
...
#                if len(winning_node_indices) > 0:
#                    print('winning_node_indices[0] = ' + str(winning_node_indices[0]))
            print('len(predictions2) = ' + str(len(predictions2)))
            winning_node_indices2 = np.where(predictions2 == node_count)  # same as above, but for sklearn-som
            print('len(winning_node_indices2) [again] = ' + str(len(winning_node_indices2)))
            print('np.array(winning_node_indices2)[0] [again] = ')# + str(winning_node_indices))
            print(np.array(winning_node_indices2)[0])
            if jSOM == 2 and iSOM == 4:
                print('winning_node_indices1[0][1] = ' + str(winning_node_indices1[0][1]))
...
#                    print('indices_month[0] = ' + str(indices_month[0]))
 ...
 #                   print('np.array(indices_month)[0] = ' + str(np.array(indices_month)[0]))
                if len(winning_node_indices1[0]) > 0:
...
#                        print('datetimes_full_yearmon_shared = ' + str(datetimes_full_yearmon_shared))
    ...
    #                    indices_month = np.where(datetimes_full_yearmon_shared == str(year_month_str)[0:5+1])  # in the array of ARI-filtered 6-hourly datetimes, find indices that only correspond to the current year and month
                    indices_month = np.where(datetimes_full_yearmon == str(year_month_str)[0:5+1])  # in the array of ARI-filtered 6-hourly datetimes, find indices that only correspond to the current year and month
#                        print('indices_month = ' + str(indices_month))
                    mymask = np.isin(indices_month[0], winning_node_indices1[0])  # create a mask that will highlight the indices in indices_month that are also clustered to the current SOM node
#                        print('mymask = ' + str(mymask))
...
#                    print('len(winning_node_indices_month) [after shared_datetimes filtering] = ' + str(len(winning_node_indices_month)))
            ...
            season_str_count = -1
            season_SOM1node_counts_per_SOM2node_allseasons = []
            for season_str in seasons_list:
                ...
                print('len(indices_season[0]) = ' + str(len(indices_season[0])))
#                    mymask2 = np.isin(indices_season, winning_node_indices)
                mymask2 = np.isin(indices_season[0], winning_node_indices1[0])
...
#                    winning_node_indices_season = np.array(indices_season)[mymask2]
            ...
            #    mymask3 = np.isin(indices_season[0], winning_node_indices1[0]
                datetimes_ari1_in_ari2period_season = []
                datetimes_ari1_in_ari2period_season_counts = []
                winning_nodes_ari1_in_ari2period_season = []
                season_SOM1node_counts_per_SOM2node = np.zeros((np.prod(SOMsize), 1))
                if len(winning_node_indices_season) > 0:
                    print('np.array(winning_node_indices_season)[0] = ' + str(np.array(winning_node_indices_season)))
                    for datetime_ari2 in datetimes_ari2[np.array(winning_node_indices_season)[0]]:
                        datetimes_ari1_in_ari2period = np.array(datetimes_ari1)[np.where(np.array(datetimes_ari1_myd) == datetime_ari2[0:7+1])[0]]
                        datetimes_ari1_in_ari2period_season.append(datetimes_ari1_in_ari2period)
                        datetimes_ari1_in_ari2period_count = len(datetimes_ari1_in_ari2period)
                        datetimes_ari1_in_ari2period_season_counts.append(len(datetimes_ari1_in_ari2period))
                        winning_nodes_ari1_in_ari2period = np.array(predictions1)[np.where(np.array(datetimes_ari1_myd) == datetime_ari2[0:7+1])]
                        winning_nodes_ari1_in_ari2period_season.append(winning_nodes_ari1_in_ari2period)
                        for prediction1 in winning_nodes_ari1_in_ari2period:
                            season_SOM1node_counts_per_SOM2node[prediction1-1] = season_SOM1node_counts_per_SOM2node[prediction1-1] + 1
                else:
                    datetimes_ari1_in_ari2period_season.append([])
                    datetimes_ari1_in_ari2period_season_counts.append(0)
                    winning_nodes_ari1_in_ari2period_season.append([])
                season_SOM1node_counts_per_SOM2node_allseasons.append(season_SOM1node_counts_per_SOM2node)

            #    ax = plt.hist(datetimes_ari1_in_ari2period_season_counts)
                fig, ax = plt.subplots()
                ax.hist(datetimes_ari1_in_ari2period_season_counts, bins=np.arange(24))
                title_fontsize = 28
                axis_fontsize = 18
                annot_fontsize = 14
                ax.set_xlabel('number of ' + varnames[0] + ' periods', fontsize=axis_fontsize)
                ax.set_ylabel('number of cases', fontsize=axis_fontsize)
                plt.yticks(rotation=0, fontsize=axis_fontsize)
                plt.xticks(rotation=45, ha='right', fontsize=axis_fontsize)
                plt.title('Maps from ' + varnames[1] + ' period that are represented by ' + varnames[0] + ' -- ' + varnames[1] + ' SOM node ' + str(node_count) + ' -- ' + season_str, fontsize=title_fontsize)
                plt.savefig('SOM_hist_' + varnames[0] + '_' + str(output_interval1) + 'out_num_cases_in_' + varnames[1] + '_' + str(output_interval2) + 'out_period_' + varnames[1] + 'SOMnode' + str(node_count) + '_' + season_str + '.png', bbox_inches='tight', pad_inches=0.01)

            fig = plt.figure(figsize=(50, 10))
            ax = sns.heatmap(np.squeeze(season_SOM1node_counts_per_SOM2node_allseasons), annot=True, fmt='g', xticklabels=seasons_list, yticklabels=np.arange(1, np.prod(SOMsize)+1), annot_kws={'size': annot_fontsize}, cbar_kws={'pad': 0.01})
            ax.set_xlabel('season', fontsize=axis_fontsize)
            ax.set_ylabel(varnames[1] + ' SOM node', fontsize=axis_fontsize)
            plt.yticks(rotation=0, fontsize=axis_fontsize)
            plt.xticks(rotation=45, ha='right', fontsize=axis_fontsize)
            cbar = ax.collections[0].colorbar
            cbar.ax.tick_params(labelsize=axis_fontsize)
            plt.title('Winning nodes from ' + varnames[1] + ' SOM at the same time as ' + varnames[0] + ' SOM node ' + str(node_count) + ' for ' + season_str, fontsize=title_fontsize)
            plt.savefig('SOM_heatmap_' + varnames[0] + '_' + str(output_interval1) + 'out_winning_nodes_in_' + varnames[1] + '_' + str(output_interval2) + 'out_period_' + varnames[1] + 'SOMnode' + str(node_count) + '_' + season_str + '.png', bbox_inches='tight', pad_inches=0.01)
...
##        plt.savefig('SOM_heatmap_' + myvarname + '_01H24Htrain_' + SOMsize_str + '_lr25_' + date_first + '_' + date_last + '_' + myregion + '_test2_fixedtest2_clean.png', bbox_inches='tight', pad_inches=0.01)
##        plt.savefig('SOM_heatmap_' + myvarname + '_01H24Htrain_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion + '_test2_fixedtest2_clean_sklearn_6hr.png', bbox_inches='tight', pad_inches=0.01)
#    plt.savefig('SOM_heatmap_' + varname + '_' + varname + 'train_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion_plot + '_test2_fixedtest2_clean_sklearn_6hr.png', bbox_inches='tight', pad_inches=0.01)
#
#    fig = plt.figure(figsize=(50, 10))
...
#    plt.savefig('SOM_heatmap_season_' + varname + '_' + varname + 'train_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion_plot + '_test2_fixedtest2_clean_sklearn_6hr.png', bbox_inches='tight', pad_inches=0.01)
#
#        #win_positions = []
...
#    filename_first = myfiles[0]
##        date_first = filename_first.split('/')[-1].split('.')[1][3:10+1]
##        filename_last = myfiles[varcount][-1]
#    filename_last = myfiles[-1]
##        date_last = filename_last.split('/')[-1].split('.')[1][3:10+1]
...
#    plt.savefig('SOM_' + varname + '_std_' + varname + 'train_' + SOMsize_str + '_' + date_first + '_' + date_last + '_' + myregion_plot + '_test2_fixedtest2_clean_sklearn_6hr.png')
