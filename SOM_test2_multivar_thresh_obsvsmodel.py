...
import sys
import numpy as np
from mysom2_multivar_thresh_obsvsmodel import mysom2_multivar_thresh_obsvsmodel  # my own function for training the SOM on multiple variables
...
#    date_first = filename_first.split('/')[-1].split('.')[1][3:10+1]
    if myfiles[myvar][0][0:3+1] == 'mrms':
        ...
        date_first = 'depends on filename'
...
[varSOM, dataArray_train, dataArrays, map_lats, map_lons] = mysom2_multivar_thresh_obsvsmodel(myfiles, date_first, date_last, varnames, SOMsize, mapSize, myregion, d)
...
#for myvar in range(0, len(mypaths)):
for myvar in range(0, len(dataArrays)):
...
#                ax_ = ax.contourf(map_lons, map_lats, np.transpose(varSOM_plot), cmap=my_var_cmap, vmin=my_var_vmin, vmax=my_var_vmax,
    ...
    plt.savefig('SOM_01H24Htrain_' + varnames[myvar] + '_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_obsvsmodel_' + date_first_str + '_' + date_last_str + '_' + myregion + '_test.png')#, bbox_inches='tight', pad_inches=0)
