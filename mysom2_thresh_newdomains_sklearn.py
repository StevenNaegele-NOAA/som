...
SOMsize_str = '3x2'#'4x3'#'5x5'
myregion = 'SE'
ari = '1yr'
output_interval = 24
precip_duration1 = '1hr'
precip_duration2 = '24hr'
precip_duration = precip_duration2#, precip_duration2]
...
#with open('/scratch1/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_multivar_' + myregion + '_precip_thresh_count_ari' + ari + '.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
#with open('/scratch1/BMC/wrfruc/naegele/SOM_4x3_multivar_' + myregion + '_precip_thresh_count_ari' + ari + '.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
#    varnames, myfiles, myregion, indices_ari_allvars, datetimes_ari_allvars, count_ari_allvars = pickle.load(f)
with open('/scratch1/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_' + precip_duration + 'QPE_' + myregion + '_precip_thresh_count_ari' + ari + '_' + str(output_interval) + 'hr.pkl', 'rb') as f:#'_2021.pkl', 'rb') as f:
    varname, myfiles, myregion, indices_ari, datetimes_ari, count_ari, precip_duration = pickle.load(f)
...
print('len(indices_ari' + ari + precip_duration + ') = ' + str(len(indices_ari)))
print('len(datetimes_ari' + ari + precip_duration + ') = ' + str(len(datetimes_ari)))
print('datetimes_ari = ')
print(datetimes_ari)
...
#def mysom2_multivar_thresh_newdomains_sklearn(myfiles, date_first, date_last, varnames, SOMsize, mapedges, myregion, d):
def mysom2_thresh_newdomains_sklearn(myfiles, date_first, date_last, varname, SOMsize, mapedges, myregion, d, precip_duration):
    print('Starting mysom function ...')

    if precip_duration == '1hr':
        output_interval = 6
    elif precip_duration == '24hr':
        output_interval = 24
#    varSOMs = []
#    dataArrays_allSOMs = []
...
#    varnames_file = ['precip', 'precip']  # variable names in the netCDF file (VAR...37... = 01H QPE, VAR...41... = 24H QPE)
    varnames_file = 'precip'  # variable names in the netCDF file (VAR...37... = 01H QPE, VAR...41... = 24H QPE)
...
#    varname_pickle = 'radaronly_QPE_01H_24H'  # variable name for the pickle file where output from this script will be saved
    varname_pickle = 'radaronly_QPE_' + precip_duration  # variable name for the pickle file where output from this script will be saved
...
#        print('Reading in data for variable ' + varnames[myvar])
    print('Reading in data for variable ' + varname)
...
#        for fileCount in range(0,len(myfiles[myvar])):
    for fileCount in range(0,len(myfiles[0])):
...
#            myfile_myvar = myfiles[myvar][fileCount]  # files for any variable, including the first one (in which case no files will be missing)
        myfile_myvar = myfiles[0][fileCount]  # files for any variable, including the first one (in which case no files will be missing)
...
#            print('Printing all statement')
 ...
 #   for myvar in range(0, len(varnames)):  # STILL NEEDS TO BE MORE GENERALIZED
    dataArray = []
    print('datetimes_full[0:5] = ' + str(datetimes_full[0:5]))
    print('datetimes_full[-5:-1] = ' + str(datetimes_full[-5:-1]))
    print('datetimes_myvar[0:5] = ' + str(datetimes_myvar[0:5]))
    print('datetimes_myvar[-5:-1] = ' + str(datetimes_myvar[-5:-1]))
#    datetimes_common = []
...
#        if datetimes_full[dateCount] in datetimes_myvar:  # check if the current datetime from a list of all possible datetimes
        if datetimes_full[dateCount] in datetimes_ari:  # check if the current datetime from a list of all possible datetimes
...
#                fileCount_myvar = datetimes_allvars[myvar].index(datetimes_full[dateCount])  # find out the index of the datetime for myvar that is the same as the current datetime from all datetimes
            fileCount_myvar = datetimes_myvar.index(datetimes_full[dateCount])  # find out the index of the datetime for myvar that is the same as the current datetime from all datetimes
#            fileCount_myvar = datetimes_ari.index(datetimes_full[dateCount])  # find out the index of the datetime for myvar that is the same as the current datetime from all datetimes
...
#                testvarread = xr.open_dataset(myfiles[myvar][fileCount_myvar], engine='netcdf4')#, chunks=2000)  # load the dataset into dask arrays, of size 2000 in each dimension
            testvarread = xr.open_dataset(myfiles[0][fileCount_myvar], engine='netcdf4')#, chunks=2000)  # load the dataset into dask arrays, of size 2000 in each dimension
        #    print('myfiles[0][fileCount_myvar] = ' + str(myfiles[0][fileCount_myvar]))
...
#                testvarMap = testvarread[varnames_file[myvar]][np.arange(mapbedge, maptedge, 10), np.arange(mapledge, mapredge, 10)]
            testvarMap = testvarread[varnames_file][np.arange(mapbedge, maptedge, 10), np.arange(mapledge, mapredge, 10)]
...
#            datetimes_common.append(datetimes_myvar[fileCount_myvar])
...
#    dataArray_orig = dataArray
    dataArray_orig = xr.DataArray(dataArray)
...
#    dataArray = dataArray[0:4999]#9934] # if dataArray is longer than this then it gets stuck?
    dataArray = xr.DataArray(dataArray)

    alltimes_gridpoint_min = np.nanmin(dataArray, axis=0)
    print('len(alltimes_gridpoint_min) = ' + str(len(alltimes_gridpoint_min)))
    alltimes_gridpoint_max = np.nanmax(dataArray, axis=0)
    print('len(alltimes_gridpoint_max) = ' + str(len(alltimes_gridpoint_max)))
    ...
    print('len(dataArray_normalized) [before NaN filtering] = ' + str(len(dataArray_normalized)))
    print('len(dataArray_normalized[0]) [before NaN filtering] = ' + str(len(dataArray_normalized[0])))
    print('dataArray_normalized[0][0] [before NaN filtering] = ' + str(dataArray_normalized[0][0]))
...
#    print('sys.getsizeof(varSOM) = ' + str(sys.getsizeof(varSOM)))
#    print('sys.getsizeof(predictions) = ' + str(sys.getsizeof(predictions)))
    print('sys.getsizeof(dataArray_orig) = ' + str(sys.getsizeof(dataArray_orig)))
#    print('sys.getsizeof(dataArray_train) = ' + str(sys.getsizeof(dataArray_train)))
    print('sys.getsizeof(myfiles) = ' + str(sys.getsizeof(myfiles)))
    print('sys.getsizeof(myfiles[0]) = ' + str(sys.getsizeof(myfiles[0])))
#    print('sys.getsizeof(predictions[0]) = ' + str(sys.getsizeof(predictions[0])))
    print('sys.getsizeof(dataArray_orig[0]) = ' + str(sys.getsizeof(dataArray_orig[0])))
#    print('sys.getsizeof(dataArray_train[0]) = ' + str(sys.getsizeof(dataArray_train[0])))
    print('sys.getsizeof(dataArray_orig[0][0]) = ' + str(sys.getsizeof(dataArray_orig[0][0])))
#    print('sys.getsizeof(dataArray_train[0][0]) = ' + str(sys.getsizeof(dataArray_train[0][0])))

#    print('len(varSOM) = ' + str(len(varSOM)))
#    print('len(predictions) = ' + str(len(predictions)))
    print('len(dataArray_orig) = ' + str(len(dataArray_orig)))
#    print('len(dataArray_train) = ' + str(len(dataArray_train)))
    print('len(myfiles) = ' + str(len(myfiles)))
    print('len(myfiles[0]) = ' + str(len(myfiles[0])))
#    print('len(predictions[0]) = ' + str(len(predictions[0])))
    print('len(dataArray_orig[0]) = ' + str(len(dataArray_orig[0])))
#    print('len(dataArray_train[0]) = ' + str(len(dataArray_train[0])))

#    print('type(varSOM) = ' + str(type(varSOM)))
#    print('type(predictions) = ' + str(type(predictions)))
    print('type(dataArray_orig) = ' + str(type(dataArray_orig)))
#    print('type(dataArray_train) = ' + str(type(dataArray_train)))
    print('type(myfiles) = ' + str(type(myfiles)))
    print('type(myfiles[0]) = ' + str(type(myfiles[0])))
#    print('type(predictions[0]) = ' + str(type(predictions[0])))
    print('type(dataArray_orig[0]) = ' + str(type(dataArray_orig[0])))
#    print('type(dataArray_train[0]) = ' + str(type(dataArray_train[0])))
    dataArray_normalized = [[x if not np.isnan(x) else -0.1 for x in y] for y in dataArray_normalized]  # replace any NaNs with a QPE that can never exist but is close enough to the data that it will not heavily influence training (e.g. -1)
    print('len(dataArray_normalized) [after NaN filtering] = ' + str(len(dataArray_normalized)))
    dataArray_normalized = [[x if x > -999 else -0.1 for x in y] for y in dataArray_normalized]  # replace any NaNs with a QPE that can never exist but is close enough to the data that it will not heavily influence training (e.g. -1)
    print('len(dataArray_normalized) [after large negative filtering] = ' + str(len(dataArray_normalized)))
    dataArray_normalized = [[x if x < 1e12 else -0.1 for x in y] for y in dataArray_normalized]  # replace any NaNs with a QPE that can never exist but is close enough to the data that it will not heavily influence training (e.g. -1)
    print('len(dataArray_normalized) [after large positive filtering] = ' + str(len(dataArray_normalized)))
...
#    dataArray_train = dataArray_normalized
    dataArray_train = xr.DataArray(dataArray_normalized)
...
#    varSOM = SOM(SOMsize[0], SOMsize[1], dim=len(varnames)*np.prod([math.ceil((mapredge-mapledge)/10), math.ceil((maptedge-mapbedge)/10)]), sigma=1.75, lr=0.875, max_iter=500)
#    varSOM = SOM(SOMsize[0], SOMsize[1], dim=len(varnames)*np.prod([math.ceil((mapredge-mapledge)/10), math.ceil((maptedge-mapbedge)/10)]), sigma=1.75, lr=0.125, max_iter=500)
    varSOM = SOM(SOMsize[0], SOMsize[1], dim=np.prod([math.ceil((mapredge-mapledge)/10), math.ceil((maptedge-mapbedge)/10)]), sigma=1.75, lr=0.125, max_iter=500)
...
#    varSOM.fit(~np.isnan(dataArray_train))
...
#    predictions = varSOM.predict(xr.DataArray(dataArray_train))
#    predictions = varSOM.predict(~np.isnan(dataArray_train))
#        varSOMs.append(varSOM)
#    print('predictions: ')
#    print(predictions)


#    print('len(dataArray_orig[0][0]) = ' + str(len(dataArray_orig[0][0])))
#    print('len(dataArray_train[0][0]) = ' + str(len(dataArray_train[0][0])))
#    print('type(dataArray_orig[0][0]) = ' + str(type(dataArray_orig[0][0])))
#    print('type(dataArray_train[0][0]) = ' + str(type(dataArray_train[0][0])))

    # Save important variables to a file
    print('Saving important variables to a file and returning to main script ...')
#    with open('SOM_' + SOMsize_str + '_thresh_percentile_99th_lr25_' + varname_pickle + '_' + myregion + '_vars_batch.pkl', 'wb') as f:  # Python 2: open(..., 'w')
#    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_zeros_2021_small_sklearn.pkl', 'wb') as f:  # Python 2: open(..., 'w')
#    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_2021_sklearn.pkl', 'wb') as f:  # Python 2: open(..., 'w')
#    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_sklearn.pkl', 'wb') as f:  # Python 2: open(..., 'w')
    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_sklearn_' + str(output_interval) + 'hr_test.pkl', 'wb') as f:  # Python 2: open(..., 'w')
#    with open('/scratch2/BMC/wrfruc/naegele/SOM_' + SOMsize_str + '_thresh_ari' + ari + precip_duration1 + '_ari' + ari + precip_duration2 + '_' + varname_pickle + '_' + myregion + '_vars_batch_newdomains_sklearn_lr125.pkl', 'wb') as f:  # Python 2: open(..., 'w')
#[varSOMs, dataArray, map_lats, map_lons] = mysom2_multivar(myfiles, varnames, SOMsizes, mapSize, myregion, d)
#            pickle.dump([varSOM, dataArray, map_lats, map_lons, months_doubles, years_doubles,
#                         year_month_strs, year_month_counts, mapedges, myfiles, varname, SOMsize, mapSize, myregion], f)
#        pickle.dump([varSOM, dataArrays_orig, dataArrays, dataArray_train, map_lats, map_lons, months_doubles_allvars, years_doubles_allvars,
...
#        pickle.dump([varSOM, predictions, dataArrays_orig, dataArray_train, mapedges, myfiles, varnames, SOMsize, mapedges, myregion], f)
        pickle.dump([varSOM, predictions, dataArray_orig, dataArray_train, mapedges, myfiles, varname, SOMsize, mapedges, myregion], f)
#        pickle.dump([varSOM, dataArray_orig, dataArray_train, mapedges, myfiles, varname, SOMsize, mapedges, myregion], f)  # testing if 'predictions' is consuming too much memory (for an unknown reason...)
        # To open the file later
        # with open('objs.pkl', 'rb') as f:  # Python 2: open(..., 'r') Python 2: open(..., 'rb')
        #     obj0, obj1, obj2, ... = pickle.load(f)
    print('Saving variables to a file was successful!')
...
#    return varSOM, predictions, dataArrays, map_lats, map_lons, dataArray_train
    return varSOM, predictions, dataArray, map_lats, map_lons, dataArray_train
