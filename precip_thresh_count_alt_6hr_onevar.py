...
mypath1 = '/scratch1/BMC/wrfruc/ejames/climo/mrms/01h/20*/'
mypath2 = '/scratch1/BMC/wrfruc/ejames/climo/mrms/24h/20*/'
...
print('filename_first = ' + filename_first)
...
#date_first = filename_first.split('_')[-1].split('.')[0][0:7+1]# + '-00'
date_first = filename_first.split('_')[-1].split('.')[0][0:9+1]# + '-00'
#    date_firsts_strs.append(date_first)
date_first_str = date_first
#date_first = datetime.datetime.strptime(date_first, '%Y%m%d')
date_first = datetime.datetime.strptime(date_first, '%Y%m%d%H')
date_first_alt = filename_first.split('_')[-1][0:7+1] + '12'
...
#date_first = date_first.strftime('%Y%m%d-%H')[0:7+1] + '-00'
#date_last = date_last.strftime('%Y%m%d-%H')[0:7+1] + '-18'
date_first = date_first.strftime('%Y%m%d-%H')[0:10+1]# + '-00'
date_last = date_last.strftime('%Y%m%d-%H')[0:10+1]# + '-18'
...
myregion = 'SE'#'NW'#'NE'#'NC'#'NW'#'SW'#'SE'#'SC'#str(sys.argv[1])
print('My region =', myregion)

#SOMsize_dim1 = int(sys.argv[2])
#SOMsize_dim2 = int(sys.argv[3])
SOMsize = [3, 2]#[4, 3]#[5, 5]#[SOMsize_dim1, SOMsize_dim2]
...
varname_pickle = 'MultiSensor_QPE_' + precip_duration  # variable name for the pickle file where output from this script will be saved
...
print('datetimes_myvar = ' + str(datetimes_myvar))


#for myvar in range(0, len(varnames)):

#    print('Reading in data for variable ' + varnames[myvar])
#print('Reading in data for variable ' + varnames)
...
#    print(ari10yr1hrread)
    #ari10yr1hrMap = ari10yr1hrread['precip'][np.arange(maptedge, mapbedge, 10), np.arange(mapledge, mapredge, 10)]
#    ariMap1 = ariread1['precip'][np.arange(mapbedge, maptedge, 10), np.arange(mapledge, mapredge, 10)]
#    print(ari10yr1hrMap)
    #ari10yr1hr = ari10yr1hrMap.stack(point=["x", "y"])  # stacks each col end-to-end, making one long row (elim 1st dim)
...
#    ariread2 = xr.open_dataset('/scratch1/BMC/wrfruc/ejames/climo/ari/allusa_ari_' + ari + '_' + precip_duration2 + '_3km.nc', engine='netcdf4')
    #ari10yr24hrMap = ari10yr24hrread['precip']
#    ariMap2 = ariread2['precip'][np.arange(mapbedge, maptedge, 10), np.arange(mapledge, mapredge, 10)]
    #ari10yr24hr = ari10yr24hrMap.stack(point=["x", "y"])  # stacks each col end-to-end, making one long row (elim 1st dim)
...
ariread = xr.open_dataset('/scratch1/BMC/wrfruc/ejames/climo/ari/allusa_ari_' + ari + '_' + precip_duration + '_3km.nc', engine='netcdf4')
ariMap = ariread['precip'][np.arange(mapbedge, maptedge, 10), np.arange(mapledge, mapredge, 10)]
ari_myvar = ariMap.stack(point=["x", "y"])  # stacks each col end-to-end, making one long row (elim 1st dim)
print(ari_myvar)
print('NaN count in ari' + ari + precip_duration + ' = ' + str(np.count_nonzero(np.isnan(ari_myvar))))
...
#            testvarread = xr.open_dataset(myfiles[myvar][fileCount_myvar], engine='netcdf4')#, chunks={"latitude":100})#, "longitude":50})#, chunks=100)  # load the dataset into dask arrays, of size 2000 in each dimension
        testvarread = xr.open_dataset(myfiles[0][fileCount_myvar], engine='netcdf4')#, chunks={"latitude":100})#, "longitude":50})#, chunks=100)  # load the dataset into dask arrays, of size 2000 in each dimension
...
#            testvarMap = testvarread[varnames_file[myvar]][np.arange(maptedge, mapbedge, 10), np.arange(mapledge, mapredge, 10)]
        testvarMap = testvarread['precip'][np.arange(mapbedge, maptedge, 10), np.arange(mapledge, mapredge, 10)]
...
#                ari_gridpoint = ari_allvars[myvar][gridpoint]
            ari_gridpoint = ari_myvar[gridpoint]
...
#                print('vectorOfMap[gridpoint] > ari' + ari + precip_duration + '[gridpoint]')#= ' + str(vectorOfMap[gridpoint]))
...
print('len(count_ari' + ari + precip_duration + ') = ' + str(count_ari))  # 
...
#with open('SOM_' + SOMsize_str + '_multivar_' + myregion + '_precip_thresh_count_ari' + ari + '_6hr.pkl', 'wb') as f:  # Python 2: open(..., 'w')
with open('SOM_' + SOMsize_str + '_' + precip_duration + 'QPE_' + myregion + '_precip_thresh_count_ari' + ari + '_' + str(output_interval) + 'hr.pkl', 'wb') as f:  # Python 2: open(..., 'w')
...
#    pickle.dump([varnames, myfiles, myregion, indices_ari_allvars, datetimes_ari_allvars, count_ari_allvars], f)
    pickle.dump([varname, myfiles, myregion, indices_ari, datetimes_ari, count_ari, precip_duration], f)
